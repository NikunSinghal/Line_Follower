...
class PIDTuner:
    ...
    def _show_guide(self):
        ...
        cod("  Kp=10.0  Ki=0.0  Kd=5.0   Base=450   Limit=270")
        tip("  Adjust from here using the steps above.")
        tip("  With dt≈1ms: try Kp in 5–50, Kd in 1–30 range first.")
        nl()
    ...
    def _relay_analyze(self):
        ...
        Kd_ratio = min(0.8, max(0.3, Tu / 4.0))  # Typically 0.3-0.8
        ...
        self._relay_log_line(f"  Kd/Kp ratio = {Kd_ratio:.2f}  (from Tu)", 'dim')
        self._relay_log_line("", '')
